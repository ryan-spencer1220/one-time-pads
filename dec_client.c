...
int main(int argc, char *argv[]) {
  ...
  if (messageFile == NULL) {
      error("Error opening text message file");
      return 1;
  }

  FILE *keyFile = fopen(argv[2], "r");
  if (keyFile == NULL) {
      error("Error opening decryption key file");
      return 1;
  }
  ...
  for (int i = 0; i < strlen(message); ++i){
    if ((message[i] < 'A' || message[i] > 'Z') && message[i] != ' ' && message[i] != '\n') {
      fprintf(stderr, "dec_client error: message contains bad characters\n");
      exit(1);
    }
  }
  ...
  for (int i = 0; i < strlen(key); ++i){
    if ((key[i] < 'A' || key[i] > 'Z') && key[i] != ' ' && key[i] != '\n') {
      error("dec_client error: key contains bad characters\n");
      exit(1);
    }
  }
